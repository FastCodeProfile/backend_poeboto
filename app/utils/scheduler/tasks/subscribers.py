...
class Subscribers:
    ...
    tasks_service = TasksService(SubscribersTasksRepository)

    async def run_tasks(self, ctx):
        tasks = []
        proxies = await self.proxies_service.get_proxies_for_working()
        if not proxies:
            logger.warning("Сейчас нет свободных прокси, задача подписчики")
            return
    ...
    async def execution_tasks(self, this_task: SubscribersTasks, proxy: Proxy):
        ...
        for bot in await self.bots_service.get_bots_for_working(this_task.skip_bots):
            ...
            try:
                await client.subscribe(this_task.link)
